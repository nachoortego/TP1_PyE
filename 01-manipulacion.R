...
ggplot(datos, aes(x = tiempo_residencia, y = "a")) + 
  ...
  labs(
    title = "Distribución del Tiempo de Residencia", 
  ...
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5, size = 24, color = "#4C4C4C"), # Título grande
    axis.title.x = element_text(size = 18, color = "#4C4C4C"),  # Eje X
    axis.text.x = element_text(size = 14, color = "#4C4C4C"),   # Números del eje X
    axis.title.y = element_blank(),  # Quitamos eje Y
  ...
  ggplot(df_acceso_ev, aes(x = "", y = Freq, fill = acceso_espacios_verdes)) +
    ...
    theme(
     ...
     ggplot(frecuencias_pc, aes(x = espacio, y = cantidad, fill = espacio)) +
       geom_bar(stat = "identity") +
       labs(
         title = "Espacios de prácticas corporales y de esparcimiento a menos de 500m de la vivienda",
         x = "Espacio",
         y = "Cantidad de personas"
       ) +
       theme_minimal() +
       ...
       scale_x_discrete(labels = function(x) str_wrap(x, width = 15)) +
       theme(
         plot.title = element_text(face = "bold", hjust = 0.5, size = 16, color = "#4C4C4C"),  # Título más chico
         axis.title.x = element_text(size = 12, color = "#4C4C4C"),  # Eje X
         axis.title.y = element_text(size = 12, color = "#4C4C4C"),  # Eje Y
         axis.text.x = element_text(size = 10, color = "#4C4C4C"),  # Etiquetas X
